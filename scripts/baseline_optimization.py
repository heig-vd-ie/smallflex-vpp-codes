    ...
    for year in YEARS:
        ...
        for turbine_factor in TURBINE_FACTORS:
            ...
            for model_id, second_stage in enumerate(results):
                ...
                sim_results[f"turbine_factor_{turbine_factor_str}_model_{model_id}"] = combined_results
                income_result[f"model_{model_id}"] = round(combined_results["income"].sum()/1e6, 3)
