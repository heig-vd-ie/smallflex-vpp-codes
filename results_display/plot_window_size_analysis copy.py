...
for i, year in enumerate([2022, 2023]):
    file_name = f".cache/output/market_price_window_size_analysis/{year}_results.duckdb"
    result_dict = duckdb_to_dict(file_name)
    basin_volume_expectation = result_dict["basin_volume_expectation_discrete_turbine_pump_7"]
...
fig.show()
fig.write_image(f"{plot_folder}/second_stage_presentation.png", width=width, height=height, scale=1)
