...
file_name = f".cache/output/water_level_quantile_analysis/{year}_results.duckdb"
result_dict = duckdb_to_dict(file_name)

# %% 


for i, window_size in enumerate([ 1, 6]):
    second_stage_result = result_dict[f"discrete_turbine_pump_{window_size}"]
    basin_volume_expectation = result_dict[f"basin_volume_expectation_discrete_turbine_pump_{window_size}"]
