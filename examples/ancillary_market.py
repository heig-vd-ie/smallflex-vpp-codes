...
for market in MARKET:
    output_folder = f"{file_names["output"]}/{market}"
    ...
    for year in YEAR_LIST:
        ...
        for hydro_power_mask, battery_size in pbar:
            ...
            if previous_hydro_power_mask != hydro_power_mask:
                ...
                if fig_1 is not None:
                    fig_1.write_html(f"{plot_folder}/{hydro_power_mask}_first_stage_results.html")


            second_stage_optimization_results, adjusted_income, fig_2 = second_stage_deterministic_pipeline(
                    data_config=data_config,
                    smallflex_input_schema=smallflex_input_schema,
            ...
            income_list.append((hydro_power_mask, battery_size, adjusted_income/1e3))
        ...
        results_data["adjusted_income"] = pl.DataFrame(
            income_list, schema=["hydro_power_mask", "battery_size", "adjusted_income"], orient="row"
