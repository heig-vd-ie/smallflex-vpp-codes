...
def numerical_curvature(d: str, d2: str) -> pl.Expr:
...
def calculate_curvature(df: pl.DataFrame, x_col: str, y_col_list: list[str]):
    ...
    df = df.with_columns(
        ...
        for col in y_col_list + [x_col]
            ...
            numerical_curvature(d=f"d_{y_col}", d2=f"d2_{y_col}").alias(f"k_{y_col}")
            for y_col in y_col_list
        )
