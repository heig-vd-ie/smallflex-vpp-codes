...
def missing_matrix_swissgrid(all_data):
    log, _ = load_configs()
    df0 = all_data
    df = df0.set_sorted("datetime", descending=False).upsample(time_column="datetime", every="1h").with_columns(pl.all().exclude(["datetime"]).fill_null(-1e10).fill_nan(-1e10).map_elements(lambda y: 0 if y < -1e9 else 1))
    ...
    ytick = df.with_columns(pl.Series(name="index", values=range(df.shape[0]))).with_columns(pl.col("datetime").dt.truncate(every="1y")).unique("datetime", keep="first")[["datetime", "index"]][1::]
    ...
    log.info(df0.set_sorted("datetime", descending=False).upsample(time_column="datetime", every="1h").describe())