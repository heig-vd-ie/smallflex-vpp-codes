...
def missing_matrix_gries(all_data):
    log, _ = load_configs()
    df = all_data.set_sorted("datetime", descending=False).upsample(time_column="datetime", every="1h").with_columns(pl.all().exclude(["datetime"]).fill_null(-1e10).fill_nan(-1e10).apply(lambda y: 0 if y < -1e9 else 1))
    ...
    log.info(all_data.set_sorted("datetime", descending=False).upsample(time_column="datetime", every="1h").describe())
    a = all_data.set_sorted("datetime", descending=False).upsample(time_column="datetime", every="1h").select(pl.col("datetime"))
    b = all_data.set_sorted("datetime", descending=False).select(pl.col("datetime"))
    log.info("missed dates:\n")
    log.info(set(a.to_pandas()["datetime"].to_list()) - set(b.to_pandas()["datetime"].to_list()))
    return ax
