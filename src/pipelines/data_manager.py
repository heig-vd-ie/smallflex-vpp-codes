...
class PipelineDataManager(PipelineConfig):
    ...
    def __process_timeseries_input(self, smallflex_input_schema: SmallflexInputSchema):
        ...
        market_price: pl.DataFrame = (
            ...
            .filter(c("timestamp").dt.year().is_in(self.year_list))
            .filter(c("timestamp").dt.ordinal_day() <= 365)
        )
        ...
        self.first_stage_ancillary_market_price = generate_clean_timeseries_scenarios(
        ...
        self.first_stage_discharge_volume = generate_clean_timeseries_scenarios(
            ...
            agg_type="sum",
            grouping_columns=["B", "year"],
        )
        
        self.first_stage_discharge_volume = self.first_stage_discharge_volume.with_columns(
            pl.concat_list("TΩ", "B").alias("TΩB")
        )
        
        unpowered_price = market_price.group_by("year").agg(
            c("avg")
            .quantile(0.5 + self.second_stage_quantile)
        ...
        self.unpowered_factor_price = unpowered_price.join(
            ...
            pl.concat_list("year", "B").alias("ΩB"),
    ...
    #         .filter(c("market") == self.market).with_columns(
    #             c("timestamp").dt.year().alias("year")
    #         ).filter(c("timestamp").dt.year().is_in(self.year_list))\
    #         .filter(c("timestamp").dt.ordinal_day()<=365)
