...
from data_federation.input_model.discharge_flow_historical import DischargeFlowHistorical
...
from data_federation.input_model.weather_historical import WeatherHistorical
...
from data_federation.input_model.discharge_flow_forecast import DischargeFlowForecast
...
from data_federation.input_model.weather_forecast import WeatherForecast
...
class SmallFlexPolarsModel(TypedDict, total=False):
    ...
    discharge_flow_historical: pl.DataFrame
    discharge_flow_measurement: pl.DataFrame
    hydro_power_plant: pl.DataFrame
    weather_historical: pl.DataFrame
    ...
    discharge_flow_forecast: pl.DataFrame
...
class SmallFlexInputModel(TypedDict, total=False):
    ...
    discharge_flow_historical: pt.DataFrame[DischargeFlowHistorical]
    discharge_flow_measurement: pt.DataFrame[DischargeFlowMeasurement]
    hydro_power_plant: pt.DataFrame[HydroPowerPlant]
    weather_historical: pt.DataFrame[WeatherHistorical]
    ...
    discharge_flow_forecast: pt.DataFrame[DischargeFlowForecast]
    water_basin: pt.DataFrame[WaterBasin]
    weather_measurement: pt.DataFrame[WeatherMeasurement]
    weather_forecast: pt.DataFrame[WeatherForecast]
...
class SmallflexInputSchema:
    ...
    discharge_flow_historical: pt.DataFrame[DischargeFlowHistorical] = field(default_factory=lambda: DischargeFlowHistorical.DataFrame(schema=DischargeFlowHistorical.columns).cast())
    ...
    weather_historical: pt.DataFrame[WeatherHistorical] = field(default_factory=lambda: WeatherHistorical.DataFrame(schema=WeatherHistorical.columns).cast())
    ...
    discharge_flow_forecast: pt.DataFrame[DischargeFlowForecast] = field(default_factory=lambda: DischargeFlowForecast.DataFrame(schema=DischargeFlowForecast.columns).cast())
    ...
    weather_forecast: pt.DataFrame[WeatherForecast] = field(default_factory=lambda: WeatherForecast.DataFrame(schema=WeatherForecast.columns).cast())
    ...
    def schema_to_duckdb(self, file_path: str):
        ...
        if os.path.exists(file_path):
            os.remove(file_path)
        dict_to_duckdb(self.__dict__, file_path)

    def duckdb_to_schema(self, file_path: str) -> SmallflexInputSchema:
        schema_dict = duckdb_to_dict(file_path)
