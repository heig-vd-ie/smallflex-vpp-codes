...
def read_gries_txt_data(gries_path: str, where: str = None):
    ...
    for l_index, l in enumerate(translate_tables.keys()):
        ...
        for file_name in tqdm.tqdm(file_names, desc="Read file"):
            ...
            df_temp = pl.from_pandas(pd.read_csv(file_path, sep='\s+', skiprows=1)).slice(1).with_columns(pl.concat_str(["YYYY", "MM", "DD", "HH", pl.lit("0")], separator=" ").alias("datetime").str.to_datetime("%Y %m %d %H %M")).select(fil_col.keys()).rename(fil_col)
