...
def read_gletsch_csv_data(
    hydro_mateo_path: str, years: list[str], ar_selection: bool, where: str = None
):
    """
    ...
    for year in years:
        ...
        file_names = list(
            filter(
                lambda file: "_AR.csv" in file
                if ar_selection
                else "_AR.csv" not in file,
                file_names,
            )
        )
        for file_name in tqdm.tqdm(file_names, desc="Read files of the year " + year):
            ...
            prediction_date = datetime.datetime.strptime(
                re.search("_\d+", file_name).group().split("_")[-1], "%Y%m%d%H"
            )
            if (
                pl.scan_csv(
                    file_path,
            ...
            if pl.scan_csv(
                ...
                df_temp = df_temp.with_columns(
                    pl.lit(None).alias(c).cast(pl.Float64)
                    for c in set(["column_" + str(i) for i in range(1, 7)]).difference(
                        df_temp.columns
                    )
                )
                df_temp = df_temp.with_columns(
                    pl.lit(prediction_date).alias("prediction_date")
                )
