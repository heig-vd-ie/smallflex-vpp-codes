...
def plot_market_prices(data: pl.DataFrame) -> go.Figure:
    ...
    direction_list: list[str] = sorted(
        data["direction"].unique().to_list(), reverse=True
    )

    unit = data["unit"][0]

    fig = make_subplots(
        rows=len(direction_list),
        cols=1,
        shared_xaxes=True,
        vertical_spacing=0.05,
        x_title="<b>Time<b>",
        y_title="<b>" + unit + "<b>",
        row_titles=direction_list,
    )
    ...
    for i, values in enumerate(product(year_list, direction_list)):
        ...
        plot_data: pl.DataFrame = (
            data.filter(c("year") == year)
            .filter(c("direction") == direction)
            .sort("index")
        )

        hovertemplate: str = (
            "Price: %{y:.1f} " + unit + "<br>Time: %{customdata}</b><extra></extra>"
        )

        fig.add_trace(
            go.Scatter(
                x=plot_data["index"].to_list(),
                y=plot_data["avg"].to_list(),
...
def plot_discharge_flow(data: pl.DataFrame) -> go.Figure:
    ...
    for i, year in enumerate(year_list):
        ...
        hovertemplate: str = (
            "Flow: %{y:.1f} m3/s<br>Time: %{customdata}</b><extra></extra>"
        )

        fig.add_trace(
            go.Scatter(
                x=plot_data["index"].to_list(),
                y=plot_data["value"].to_list(),
                customdata=plot_data["displayed_date"].to_list(),
                mode="lines",
                name=str(year),
                legendgroup=str(year),
...
def plot_discrete_performance_table(data: pl.DataFrame) -> go.Figure:
    ...
    flow_data: pl.DataFrame = data.pivot(
        index="head", on="state_number", values="flow"
    ).sort("head")
    electrical_power_data: pl.DataFrame = data.pivot(
        index="head", on="state_number", values="electrical_power"
    ).sort("head")
    fig = make_subplots(
        rows=2,
        cols=1,
        ...
        vertical_spacing=0.05,
        x_title="<b>head [m]<b>",
        row_titles=["Flow [m3/s]", "Power [MW]"],
    )

    for i, state in enumerate(data["state_number"].unique().to_list()):
        hovertemplate: str = (
            "Flow: %{y:.1f} [m3/s]<br>Head: %{x:.1f} m</b><extra></extra>"
        )
        fig.add_trace(
            go.Scatter(
                x=flow_data["head"].to_list(),
                y=flow_data[state].to_list(),
                mode="lines",
                name=str(state),
                legendgroup=str(state),
        ...
        hovertemplate: str = (
            "Electrical Power: %{y:.1f} [MW]<br>Head: %{x:.1f} m</b><extra></extra>"
        )
        fig.add_trace(
            go.Scatter(
                x=electrical_power_data["head"].to_list(),
                y=electrical_power_data[state].to_list(),
                mode="lines",
                name=str(state),
                legendgroup=str(state),
                line=dict(color=COLORS[i]),
...
def plot_continuous_performance_table(data: pl.DataFrame) -> go.Figure:
    ...
    for i, head in enumerate(data["head"].unique().to_list()):
        ...
        hovertemplate: str = (
            "Flow: %{x:.2f} m3/s<br>Power: %{y:.1f} MW </b><extra></extra>"
        )
        fig.add_trace(
            go.Scatter(
                x=plot_data["flow"].to_list(),
                y=plot_data["electrical_power"].to_list(),
                mode="lines",
                name=str(head),
                legendgroup=str(head),
                line=dict(color=COLORS[i % len(COLORS)]),
                hovertemplate=hovertemplate,
                showlegend=True,
            )
...
def plot_power(data: pl.DataFrame) -> go.Figure:
    ...
    for i, year in enumerate(year_list):
        ...
        plot_data: pl.DataFrame = data.filter(c("year") == year).sort("index")
        hovertemplate: str = (
            "Power: %{y:.1f} MW<br>Time: %{customdata}</b><extra></extra>"
        )

        fig.add_trace(
            go.Scatter(
                x=plot_data["index"].to_list(),
                y=plot_data["avg_active_power"].to_list(),
                customdata=plot_data["displayed_date"].to_list(),
                mode="lines",
                name=str(year),
                legendgroup=str(year),
...
def plot_basin_height(data: pl.DataFrame) -> go.Figure:
    ...
    ticks_df = plot_data.gather_every(
        n=plot_data.height // ticks_nb, offset=plot_data.height // (ticks_nb * 2)
    )
...
# def plot_basin_height_volume_table(
...
#     for data in small_flex_input_schema.water_basin.filter(
#         c("uuid").is_in(water_basin_fk)
#     ).to_dicts():
#         plot_data = basin_height_volume_table.filter(
#             c("water_basin_fk") >= data["uuid"]
#         )
#         if data["height_min"]:
...
#         hovertemplate: str = (
#             "Height: %{x:.1f} masl<br>: Volume: %{y:.1f} Mm^3</b><extra></extra>"
#         )
...
#         fig.add_trace(
#             go.Scatter(
#                 x=plot_data["height"].to_list(),
#                 y=(plot_data["volume"]).to_list(),
#                 mode="lines",
#                 line=dict(color=COLORS[0]),
#                 showlegend=True,
#                 hovertemplate=hovertemplate,
...
def update_figure_layout(fig: go.Figure, data: pl.DataFrame) -> go.Figure:
    ...
    ticks_df = ticks_df.gather_every(
        n=ticks_df.height // 12, offset=ticks_df.height // 24
    ).with_columns(
...
def plot_forecast(
    ...
    fig_2 = make_subplots(
        ...
        shared_xaxes=True,
        vertical_spacing=0.05,
        subplot_titles=("Mean error [%]", "Sum error", "Number of error"),
    )
    ...
    for i in tqdm.tqdm(range(n_rows - 1), desc=f"Plotting {title} forecast"):
        ...
        measurement_np = (
            measurement_df.filter(c("timestamp") > actual_date)
            .slice(0, len(mean_forecast))["value"]
            .to_numpy()
        )

        relative_error_np = relative_error_within_boundaries(
            measurement_np, z_score_min, z_score_max
        )[: 24 * nb_days].reshape(nb_days, 24)
        error_np = error_within_boundaries(measurement_np, z_score_min, z_score_max)[
            : 24 * nb_days
        ].reshape(nb_days, 24)
        ...
        datetime_list = pl.datetime_range(
            ...
            actual_date + timedelta(days=5) - timedelta(hours=1),
            timedelta(hours=1),
            eager=True,
        ).to_list()
        ...
        fig.update_traces(
            selector=dict(legendgroup=f"day{i}"),
            legendgrouptitle_text=actual_date.strftime("%Y-%m-%d"),
        )

    for i in range(nb_days):
        fig_2.add_trace(
            go.Box(
                y=mean_error_np[:, i] * 100,
                name=f"day {i+1}",
                showlegend=False,
                line=dict(color=COLORS[i]),
            ),
...
def plot_historical_scenarios(
    ...
    fig = make_subplots(
        ...
        subplot_titles=loc_list,
        x_title=x_axis,
    )
    loc_list = data["location"].unique().to_list()
    for i, location in enumerate(loc_list):
        data_loc = data.filter(c("location") == location).pivot(
            index=x_axis, on="year", values=values
        )
        for j, col in enumerate(data_loc.columns[1:]):
            fig.add_trace(
                go.Scatter(
                    x=data_loc[x_axis].to_list(),
                    y=data_loc[col].to_list(),
                    mode="lines",
                    name="Measurement",
        ...
        data_loc = data_loc.with_columns(
            ...
            pl.concat_list(pl.all().exclude(x_axis))
        ...
        fig.add_trace(
            go.Scatter(
                x=data_loc[x_axis].to_list(),
                y=data_loc["upper_bound"].to_list(),
                mode="lines",
                name="z_score",
                legendgroup="z_score",
                showlegend=i == 0,
        ...
        fig.add_trace(
            go.Scatter(
                x=data_loc[x_axis].to_list(),
                y=data_loc["lower_bound"].to_list(),
                mode="lines",
                name="z_score",
                legendgroup="z_score",
                showlegend=False,
...
def plot_forecast_vs_measurement(input_timeseries: pl.DataFrame):

    fig = make_subplots(
        rows=3,
        cols=1,
        shared_xaxes=True,
        row_titles=("Irradiation", "Wind", "Discharge volume"),
        vertical_spacing=0.02,
    )
    fig.add_trace(
        go.Scatter(
            x=input_timeseries["timestamp"],
            y=input_timeseries["irradiation_mean_forecast"],
            name="Mean forecast",
            line=dict(color="red"),
        ),
        row=1,
    ...
    fig.add_trace(
        go.Scatter(
            x=input_timeseries["timestamp"],
            y=input_timeseries["irradiation"],
            name="Measurement",
            line=dict(color="blue")
        ),
        row=1,
    ...
    fig.add_trace(
        go.Scatter(
            x=input_timeseries["timestamp"],
            y=input_timeseries["wind_mean_forecast"],
            name="Wind Forecast",
            showlegend=False,
            line=dict(color="red"),
        ),
    ...
    fig.add_trace(
        go.Scatter(
            x=input_timeseries["timestamp"],
            y=input_timeseries["discharge_volume_0_mean_forecast"],
            name="Discharge volume Forecast",
            showlegend=False,
            line=dict(color="red"),
        ),
        row=3,
    ...
    fig.add_trace(
        go.Scatter(
            x=input_timeseries["timestamp"],
            y=input_timeseries["discharge_volume_0"],
            name="Discharge volume Measurement",
            showlegend=False,
            line=dict(color="blue"),
        ),
        row=3,
    ...
    fig.update_layout(
        height=900,
        width=1200,
        title_text="Weather and Discharge Flow Forecast vs Measurement",
    )
    return fig
